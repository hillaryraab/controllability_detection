...
#WASI
_Sanity check to confirm no age effects on IQ.
```{r initialize WASI}
#load subject log
...
## Run difficulty
_Does performance on state predictions differ on the easier vs. harder runs of the task?
...
_First, we wanted to examine how accuracy on the state prediction trials changed as a function of age, trial, condition, working memory, and exploratory trial.
...
## Run difficulty
```{r no effect of run difficulty on condition prediction accuracy}
#Does performance decrease when state transitions become more probabilistic? 
...
```{r condition pred model 3 without wm}
#final model without working memory
...
_Is accuracy on state predictions predictive of accuracy on condition predictions across age?
...
```{r WM mediation b/n age and state and condition accuracy}
library("mediation")
set.seed(2021)
...
explicitKnowledge <- select(explicitKnowledge,"SubjID","Total_explicitKnowledge")
